...
func (c *nodeLabelsTaintsCheck) Run(objects *kube.Objects) ([]checks.Diagnostic, error) {
	...
	for _, node := range objects.Nodes.Items {
		...
		for labelKey := range node.Labels {
			if !isKubernetesLabel(labelKey) && !isBKELabel(labelKey) {
...
func isBKELabel(key string) bool {
	// DOKS labels use the doks.bizflycloud.com namespace. Assume all such
	// labels are set by DOKS.
	if strings.HasPrefix(key, "bke.bizflycloud.vn/") {
		return true
	}

	if strings.HasPrefix(key, "topology.volume.csi.bizflycloud.vn/") {
		return true
	}
...
func isBKETaint(taint corev1.Taint) bool {
